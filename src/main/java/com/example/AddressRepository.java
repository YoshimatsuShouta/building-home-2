...
public class AddressRepository {
	public void insertToAddress(List<Original> originalList) {
		...
		String sql = "INSERT INTO address (name, name_kana, name_rome, latitude, longitude, prefecture_id, municipality_id) SELECT ?, ?, ?, ?, ?, ?, ? WHERE NOT EXISTS (SELECT name FROM address WHERE name = ? AND prefecture_id = ? AND municipality_id = ?);";
